...
1. `goconst.Const(ptr)` takes a pointer to an object and stores a weak reference to it along with a JSON-encoded hash of its current state.
2. A background goroutine polls all tracked objects every minute.
